...
| Self-Consistency k=3 | Implemented | Three independently sampled candidates, Lab B3 stance-signature majority vote, then critic review in `self_consistent_answer` |
...
`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_BASE_URL` are configured.
Without credentials the application still runs, but no remote trace can be shown.

The exact reuse/adaptation lineage for Labs B1-B4, `llm_helpers.py`, and the course
MCP server is documented in `docs/lab_adaptation.md`.
